...
import styles from './NaverCallbackPage.module.css';
...
const NaverCallbackPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const response = await fetch('/api/auth/social/naver/login', {
          method: 'POST',
        ...
        if (!response.ok) {
          try {
            ...
            setErrorMessage(translateSocialAuthError(error.message ?? 'Naver 로그인에 실패했습니다.'));
          } catch {
            setErrorMessage('Naver 로그인에 실패했습니다.');
          }
          return;
        }
